    ...
    stages {
        ...
        stage('Run and Persist Database') {
            steps {
                ...
                sh 'docker exec my-mongo-container mongoimport --host localhost --port 27017 --db ecom --collection products --file /data/db/data.json --jsonArray'
            }
        }
        ...
        stage('Deploy Frontend') {
            steps {
                sh 'docker run -d -p 3001:3000 --name my-react-container --link my-express-container:backend -e REACT_APP_BACKEND_URI=http://44.220.249.251:3000 my-react-image'
            }
        }
    }
